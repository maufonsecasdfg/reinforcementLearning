...
def run_experiment(runs, steps, k, bandit_type, true_reward_mean, true_reward_std, arm_dist_std, nonstat_bandit_rand_walk_update_std, 
                    agent_method, agent_alpha, agent_Q_1=None, agent_epsilon=None, agent_c=None):
    ...
    experiment_parameters = pd.DataFrame(pd.Series({
        ...
        'true_reward_mean' : true_reward_mean,
        ...
        'agent_Q_1' : str(agent_Q_1),
        'agent_c' : agent_c
    })).transpose()

    for j in range(runs):
        distributions = [norm]*k
        dist_params = [[norm(true_reward_mean,true_reward_std).rvs(),arm_dist_std] for i in range(k)]
        ...
        agent = Agent(k = k, method=agent_method, alpha=agent_alpha, Q_1=agent_Q_1, epsilon=agent_epsilon, c=agent_c)
